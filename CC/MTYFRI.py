...
def func():
    n, k = int_arr()
    arr = list(int_arr())
    motu_picks = sorted([arr[i] for i in range(0, n, 2)])
    tomu_picks = sorted([arr[i] for i in range(1, n, 2)])
    i = 0
    while i < min(k, n // 2):
        if motu_picks[-1 - i] > tomu_picks[i]:
            tomu_picks[i], motu_picks[-1 - i] = (
                motu_picks[-1 - i],
                tomu_picks[i],
            )
        else:
            break
        i += 1
    return sum(tomu_picks) > sum(motu_picks)
