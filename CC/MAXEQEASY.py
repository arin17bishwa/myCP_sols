...
def func():
    n = iin()
    freq = Counter(int_arr())
    if len(freq) == 1:
        return (n * (n - 1)) // 2
    sorted_freq_arr = sorted([(j, i) for i, j in freq.items()], reverse=True)
    if sorted_freq_arr[0][1] == 0:
        mx_item = sorted_freq_arr[1][1]
    else:
        mx_item = sorted_freq_arr[0][1]
    freq[mx_item] += freq[0]
    freq[0] = 0
    return sum(map(lambda x: (x * (x - 1)) // 2, freq.values()))
