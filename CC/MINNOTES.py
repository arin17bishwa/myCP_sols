...
def func():
    ...
    gcd0 = suffix_gcd[1]
    gcd1 = prefix_gcd[-2]
    ans = min(suffix_sum[1] // suffix_gcd[1], prefix_sum[-2] // prefix_gcd[-2]) + 1
    mx_gcd = max(gcd0, gcd1)

    for i in range(1, n - 1):
        curr_gcd = gcd(prefix_gcd[i - 1], suffix_gcd[i + 1])
        ans = min(ans, ((prefix_sum[i - 1] + suffix_sum[i + 1]) // curr_gcd) + 1)
        mx_gcd = max(mx_gcd, curr_gcd)
