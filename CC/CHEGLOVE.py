...
def func():
    n = iin()
    fingers = list(int_arr())
    sheaths = list(int_arr())
    front = all(finger <= sheath for finger, sheath in zip(fingers, sheaths))
    back = all(finger <= sheath for finger, sheath in zip(fingers, reversed(sheaths)))
    if front and back:
        return "both"
    if front:
        return "front"
