...
def difference(s: str, t: str) -> int:
    return sum(i != j for i, j in zip(s, t))


def make_sequence(s: str, n: int) -> str:
    return s * (n // 3) + s[: n % 3]
...
def func():
    ...
    return min(
        min(
            difference(wall, make_sequence(s, n)),
            difference(wall, make_sequence(s[::-1], n)),
        )
        for s in ["RGB", "GBR", "BRG"]
    )
