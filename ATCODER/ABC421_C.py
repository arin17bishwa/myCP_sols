...
def func():
    ...
    permitted_pos_1 = [i << 1 for i in range(n)]
    permitted_pos_2 = [(i << 1) | 1 for i in range(n)]

    return min(
        sum(abs(pos_a[i] - permitted_pos_1[i]) for i in range(n)),
        sum(abs(pos_a[i] - permitted_pos_2[i]) for i in range(n)),
    )
