from typing import Iterable, Callable, DefaultDict, List
...
def func():
    ...
    for idx in range(1, n + 1):
        manager = iin()
        employees[manager].append(idx)
    return bfs_max_depth(tree=employees) - 1


def bfs_max_depth(tree: DefaultDict[int, List[int]]):
    queue = deque([-1])
    ans = 0
    while queue:
        ans += 1
        for _ in range(len(queue)):
            curr = queue.popleft()
            for _employee in tree[curr]:
                queue.append(_employee)
    return ans
